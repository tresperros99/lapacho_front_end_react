...
interface SelectTipoIngresoComponentProps {
  fullWidth: boolean;
  setTipoIngresoSeleccionado: React.Dispatch<
    React.SetStateAction<TiposIngreso | null>
  >;
}

const SelectTipoIngresoComponent: React.FC<SelectTipoIngresoComponentProps> = ({
  fullWidth,
  ...
  const handleTipoIngresoChange = (event: SelectChangeEvent<number>) => {
    ...
    const tipoIngresoSeleccionado = tiposIngresos.find(
      (tipo) => tipo.idTipo === tipoIngresoId,
    );
