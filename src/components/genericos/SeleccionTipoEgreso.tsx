...
interface SelectTipoEgresoComponentProps {
    fullWidth: boolean;
    setTipoEgresoSeleccionado: React.Dispatch<React.SetStateAction<TiposEgreso | null>>;
}

const SelectTipoEgresoComponent: React.FC<SelectTipoEgresoComponentProps> = ({ fullWidth, setTipoEgresoSeleccionado }) => {
    ...
    const handleTipoIngresoChange = (event: SelectChangeEvent<number>) => {
        ...
        const tipoEgresoSeleccionado = tiposEgresos.find(tipo => tipo.idTipo === tipoEgresoId);
    ...
    return (
        <Box>
            ...
            >
                {tiposEgresos.length > 0 ? tiposEgresos.map((tipo, index) => (
