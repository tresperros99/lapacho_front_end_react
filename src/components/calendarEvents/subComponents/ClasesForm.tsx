...
const ClasesForm: React.FC<ClaseFormProps> = ({ initialValues, onSubmit, onClose }) => {
    ...
    useEffect(() => {
        const fetchProfesores = async () => {
            const data = await getNominaProfesores();
            if (data && data.profesoresFormateado) {
                setProfesores(data.profesoresFormateado);
            }
        };
        fetchProfesores();
                ...
                <Form>
                    <Box display="flex" flexDirection="column" gap={2}>
                        ...
                        <FormControl fullWidth>
                            ...
                            <Field name="idProfesor" as={Select} labelId="profesor-label" label="Profesor">
                                {profesores.map(profesor => (
                                    <MenuItem key={profesor.idProfesor} value={profesor.idProfesor}>
                        ...
                        <Field name="inicio" as={TextField} label="Inicio" type="datetime-local" InputLabelProps={{ shrink: true }} />
                        <ErrorMessage name="inicio" component="div" />

                        <Field name="fin" as={TextField} label="Fin" type="datetime-local" InputLabelProps={{ shrink: true }} />
