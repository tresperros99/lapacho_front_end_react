...
import { getCuotasPagadasDelMes } from '../../../api/ApiCuotas';
...
const CuotasPagadasDelMes = () => {
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [cuotasPagadasDelMes, setCuotaPagadasDelMes] = useState<CuotasPendiente[]>([]);
    ...
    const fetchData = async () => {
        try {
            const response = await getCuotasPagadasDelMes();
            if (response) {
                if (Boolean(response.status)) {
                    const sortedData = sortData(response.cuotasPendientes);
                    setCuotaPagadasDelMes(sortedData);
                } else {
                    setCuotaPagadasDelMes([]);
                }
            } else {
    ...
    return (
        <ContainerComponent>
            <Typography textAlign={'center'} variant='h4' marginBottom={2}>Panel de Cuotas Pagadas del mes</Typography>
                    ...
                    <TableFooter>
                        <TableRow>
                            <TablePagination
                                ...
                                colSpan={3}
                                count={cuotasPagadasDelMes.length}
...
export default CuotasPagadasDelMes;
