...
const CuotasPendientes = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (response) {
        if (response.status) {
          const cuotasPendientes = response.cuotas.filter(
            (cuota) => cuota.fechaPago === null,
          );
  ...
  const handlePagoCuota = async (pagarCuotaPendienteSocioDto: any) => {
    const pagarCuotaPendienteSocio = await postPagarCuotaPendienteSocio(
            ...
            {cuotasPendientes.map((cuota) => (
              <TableRow key={cuota.idCuotaSocio}>
                ...
                <TableCell align="right">
                  {capitalizePorPalabra(cuota.cuotaMes)}
                </TableCell>
                <TableCell align="right">
                  {cuota.fechaVencimiento.toLocaleDateString()}
                </TableCell>
                <TableCell align="right">
                  {separadorMiles(cuota.montoCuota, true)} Gs.
                </TableCell>
                <TableCell align="right">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handlePagoCuota({
                        ...
                        descripcionPago: `cuota pagada ${cuota.cuotaMes}`,
                        montoAbonado: cuota.montoCuota,
                        numeroCedula: cuota.cedula,
                        nroFactura: "XXXXX",
                      })
                    }
                  >
