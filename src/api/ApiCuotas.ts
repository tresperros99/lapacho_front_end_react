...
import { ListadoCuotasPagadasSocio } from "../models/responses/cuotas/ListadoCuotasPagadasSocio.response";

const { cuotas } = appConfig


export const getCuotasPagadasSocio = async (nroCedula: string, annio: string) => {
    const url = cuotas.getListadoCuotasPagadasSocio
    const params = {
        numero_cedula: nroCedula,
        annio,
    }
    const cuotasPagadasSocioResp = await axiosInstance.get<ListadoCuotasPagadasSocio>(url,{params});
