...
export const getCuotasPendientesSocio = async (nroCedula: string, annio: string) => {
    const url = cuotas.getListadoCuotasPendientesSocio
    const params = {
        numero_cedula: nroCedula,
        annio,
    }
    const cuotasPagadasSocioResp = await axiosInstance.get<ListadoCuotasPendientesSocio>(url,{params});
...
export const getCuotasPendientesDelMes = async () => {
    ...
    const listadocuotasPendientesMesResp = await axiosInstance.get<ListadoCuotasPendientesDelMesResponse>(url);
...
export const getCuotasPagadasDelMes = async () => {
    ...
    const listadoCuotasPagadasDelMesResp = await axiosInstance.get<ListadoCuotasPendientesDelMesResponse>(url);
...
export const postPagarCuotaPendienteSocio = async (pagarCuotaPendienteSocioDto:PagarCuotaPendienteSocioDto) => {
    const url = cuotas.pagarCuotaSocio

    const pagarCuotaSocioResp = await axiosInstance.post<CuotaPagadaResponse>(url,pagarCuotaPendienteSocioDto);
...
export const postPagoCuotasSociosVarias = async (pagoCuotasSocioVariasDto:PagoCuotaSociosVariasDto) => {
    const url = cuotas.pagarCuotaSocio

    const pagarCuotasSocioResp = await axiosInstance.post<CuotasSociosPagadasResponse>(url,pagoCuotasSocioVariasDto);
