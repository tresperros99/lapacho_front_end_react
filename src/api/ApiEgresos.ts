...
export const postCargarEgreso = async (nuevoEgreso: NuevoEgresoDto) => {
  ...
  const cargarEgresoResp = await axiosInstance.post<EgresoCargadoResponse>(
    url,
    nuevoEgreso,
  );
  if (cargarEgresoResp) {
...
export const getListadoEgresosUsuarios = async (
  fechaDesde: string,
  fechaHasta: string,
  pagina: string,
) => {
  try {
    const url = appConfig.egresos.obtenerEgresos
      .replace("${fechaDesde}", fechaDesde)
      .replace("${fechaHasta}", fechaHasta)
      .replace("${pagina}", pagina);
    const responseListadoEgresos =
      await axiosInstance.get<ListadoEgresosUsuarioResponse>(url);
...
export const getExcelEgresos = async (
  fechaDesde: string,
  fechaHasta: string,
) => {
  try {
    const url = appConfig.egresos.obtenerExcelEgresos
      .replace("${fechaDesde}", fechaDesde)
      .replace("${fechaHasta}", fechaHasta);
